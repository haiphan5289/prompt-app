...
parameters:
  ...
  - name: referenceUseCase
    description: An existing UseCase name or file path to use as a reference workflow (e.g., FetchCartDetail or AppFeatures/CTCorePayment/CTCorePayment/Domain/UseCases/CRCheckoutUseCase.swift). The AI will read this UseCase across all 6 layers and mirror its exact code style, naming conventions, error handling, and patterns.
    required: false
---

# [AI] Auto-generate a UseCase through the layers: Targets, Services, Repositories, UseCase, ViewModel

## ⚠️ Pre-Flight Check — Ask For Missing Inputs Before Proceeding

**Before generating any code**, the AI MUST validate that all required parameters are present.

### Required Parameters Checklist

| # | Parameter | Status Check |
|---|-----------|-------------|
| 1 | `USECASE_NAME` | Must be provided — cannot be inferred |
| 2 | `INPUT_PARAM` | Must be provided — e.g. `String`, `Int`, `CreateOrderRequest` |
| 3 | `OUTPUT_PARAM` | Must be provided — e.g. `OrderStatistics` |
| 4 | `ENDPOINT_PATH` | Must be provided — e.g. `"v1/orders/statistics"` |
| 5 | `HTTP_METHOD` | Must be provided — `get`, `post`, `put`, or `delete` |
| 6 | `RESPONSE_MODEL` | Must be provided — e.g. `OrderStatisticsResponseModel` |
| 7 | `TARGET_CLASS` | Must be provided or ask user for the file name |
| 8 | `SERVICE_CLASS` | Must be provided or ask user for the file name |
| 9 | `REPOSITORY_CLASS` | Must be provided or ask user for the file name |
| 10 | `USECASE_CLASS` | Must be provided or ask user for the file name |
| 11 | `MODEL_CLASS` | Must be provided or ask user for the file name |
| 12 | `VIEWMODEL_CLASS` | Must be provided or ask user for the file name |

### Missing Input Rules

1. **If any required parameter is missing** — STOP and ask the user to provide it before generating any code.
2. **If multiple parameters are missing** — Ask for ALL missing ones in a single message, not one at a time.
3. **If `GENERIC_MODEL` is missing** — Auto-detect from module location (CorePayment → `CRModelCommon`, VEH → `BaseResponseModel`, POS → no wrapper). If uncertain, ask.
4. **If `ENDPOINT_CLASS` is not provided** — Default to `CRNetworkHelper` for CorePayment, otherwise ask.
5. **If `REFERENCE_USECASE` is not provided** — This is optional; proceed without it using the generic template.
6. **Never guess or assume** values for required parameters — always ask the user.

### Ask Template (use when inputs are missing)

```
Before I generate the UseCase, I need a few more details:

❓ Missing required parameters:
- USECASE_NAME: [not provided]
- INPUT_PARAM: [not provided]
- ...

Please provide the missing values above and I'll generate the complete 6-layer implementation.
```
...
**Reference UseCase:** {REFERENCE_USECASE} *(optional — used as workflow reference)*  
...
| `{REFERENCE_USECASE}` | *(optional)* Existing UseCase name or file path to mirror | FetchCartDetail or path/to/File.swift |

### 🔍 **REFERENCE_USECASE Behaviour:**
When `{REFERENCE_USECASE}` is provided, the AI **MUST**:
1. **Search** for the referenced UseCase across all 6 layers (Target, Service, Repository, UseCase, ViewModel, Model)
2. **Read** its full implementation to understand the module's code style and patterns
3. **Mirror** naming conventions, parameter mapping, error handling, and structure exactly
4. **Prefer** the reference pattern over the generic template when there is any conflict

| Scenario | Value | AI Behaviour |
|---|---|---|
| By UseCase name | `FetchCartDetail` | Searches all 6 layers for `FetchCartDetail`, reads its implementation, mirrors the pattern |
| By file path | `AppFeatures/CTCorePayment/.../CRCheckoutUseCase.swift` | Opens the file, reads all UseCases inside, uses them as canonical reference |
| Not provided | *(omitted)* | Falls back to the generic template defined in this prompt |
...
- ViewModel Class: {VIEWMODEL_CLASS}
- Generic Model: {GENERIC_MODEL}
- Reference UseCase: {REFERENCE_USECASE} (optional — read this first, then mirror its patterns)
...
{REFERENCE_USECASE}: FetchCartDetail
...
Fill in all required parameters using this format:
...
> **Tip:** `REFERENCE_USECASE` is optional but highly recommended. Providing it ensures the generated code matches the existing conventions in your module.

---

### **Step 2 — Provide the JSON Response (optional but recommended)**

Paste the expected API response so the AI can generate accurate model properties:
...
### **Step 3 — Let the AI generate all 6 layers**

The AI will automatically insert code into these 6 existing files (never creating new files):

| Layer | File Modified |
|-------|--------------|
| 1. NetworkHelper | `{ENDPOINT_CLASS}` — add `Api.{USECASE_NAME}` constant |
| 2. Targets | `{TARGET_CLASS}` — add `{USECASE_NAME}Target` struct |
| 3. Services | `{SERVICE_CLASS}` — add `{USECASE_NAME}(input:)` method |
| 4. Repositories | `{REPOSITORY_CLASS}` — add pass-through method |
| 5. UseCase | `{USECASE_CLASS}.swift` — add `CR{USECASE_NAME}UseCase` class |
| 6. ViewModel | `{VIEWMODEL_CLASS}` — add `execute{USECASE_NAME}(input:)` method |

Plus the **Model** in `{MODEL_CLASS}.swift` → total **7 existing files modified**.
...
```
[AI] Auto-generate a UseCase through the 6-layer architecture:
...
Parameters: ["order_id": input]

If REFERENCE_USECASE is provided, first read its implementation across all 6 layers, then follow the same workflow.
Only modify existing files. Never create new files.
```
...
|-----------|----------|---------|
| `USECASE_NAME` | ✅ | `FetchOrderStatistics` |
| `INPUT_PARAM` | ✅ | `String`, `Int`, `CustomModel` |
| `OUTPUT_PARAM` | ✅ | `OrderStatistics` |
| `ENDPOINT_PATH` | ✅ | `"v1/orders/statistics"` |
...
| `MODEL_CLASS` | ✅ | `CRCheckOutModel` |
| `VIEWMODEL_CLASS` | ✅ | `CRCheckoutPageViewModel` |
| `GENERIC_MODEL` | ✅ | `CRModelCommon` *(auto-detect)* |
| `REFERENCE_USECASE` | ⬜ optional | `FetchCartDetail` or file path |
...
**� Reference UseCase**: If `{REFERENCE_USECASE}` is provided, read it first across all 6 layers before generating any code. Use it as the canonical workflow reference.
